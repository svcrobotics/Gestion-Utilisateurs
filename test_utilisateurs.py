...
def run():
    ...
    for line in f.readlines():
        ...
        read_groupe(groupe)
        print(path_du_groupe_exist, groupe_exist)
        read_user(user)
...
def test_read_groupe_exist():
    assert read_groupe("system") == (True, True)

def test_read_groupe_inexistant():
    assert read_groupe("toto") == (False, False)

def test_read_groupe_special():
    assert read_groupe("-") == (False, False)
...
def test_read_user_exist():
    assert read_user("jmtlg_planteur") == (True, "/home/system/jmtlg_planteur")

def test_read_user_inexistant():
    assert read_user("") == (False, "")
...
def test_create_user_existant():
    assert create_user(True, "jmtlg_planteur", "brioche", "system") == True

def test_create_user_inexistant():
    assert create_user(False, "manolo", "toto1234", "-") == False

def creer_path_user(user, user_exist):
    if user_exist:
        try:
            os.mkdir(path2 + '/' + user)
        except FileExistsError:
            print("Le path de l'utilisateur existe déjà.")

def test_creer_path_user():
    assert creer_path_user("veterrini_havelock",True) == "Le path de l'utilisateur existe déjà."
...
def archiver_user(user, path_csv):
    '''
    ...
    return user_exist, path_du_groupe_exist

def test_archiver_user():
    path_csv = "/home/-/jmtlg_planteur"
    assert archiver_user("jmtlg_planteur", path_csv) == (False, False)

def deplacer_user(user, path_init, new_path, path):
    '''
...
'''
def test_deplacer_user():
    user = "ridculle_mustrum"
    path_init = "/home/direction/ridculle_mustrum"
    new_path = "/home/-/ridculle_mustrum"
    path = "/home/-/"
    assert deplacer_user(user, path_init, new_path, path) == (path + user)
'''
#########################################################
